const palantirDb = {
    ...
    clearVolatile: () => {
        ...
        try {
            ...
            //palantirDb.db.prepare("DELETE From Lobbies WHERE json_extract(Lobby, '$.ID') NOT IN (SELECT DISTINCT json_extract(Status, '$.LobbyID') from Status WHERE json_extract(Status, '$.LobbyID') IS NOT NULL);").run();
            result = { valid: true };
        }
        catch{
            palantirDb.close();
